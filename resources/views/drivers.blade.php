...
@section('content')
{{-- AppBridge v4 Title Bar for Shopify admin (menu=1) --}}
@if((int) request('menu') === 1)
{{-- AppBridge v4 App Nav - Global Navigation Menu --}}
@include('partials.app_nav')

<s-page heading="Drivers">
    {{-- Primary action button - navigates to Kitchen --}}
    <s-button slot="primary-action" onclick="navigateToPage('/kitchen/ADMIN?menu=1')">Kitchen</s-button>
        ...
        @foreach($storesData as $storeIndex => $storeGroup)
            <div class="row mt-4">
                <div class="col-12">
                    ...
                    <h5 class="pb-2 mb-3">
                        🏪 {{ $storeGroup['store']->name }} <span class="badge bg-secondary">{{ $storeGroup['locations']->count() }}</span>
                    </h5>
                </div>
            </div>
