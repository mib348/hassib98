...
// dd($products);
    ...
    public function getTheme(Request $request){
        ...
        if ($response['errors']) {
            ...
            $content = str_replace('</div></div><button data-product-id', '</div></div><input type="hidden" name="properties[max_quantity]" value="{{ qtyValue }}"><input type="hidden" name="properties[location]" value="{{ location }}"><input type="hidden" name="properties[date]" value="{{ date }}"><input type="hidden" name="properties[day]" value="{{ day_name }}"><input type="hidden" name="properties[no_station]" value="{{ no_station }}"><input type="hidden" name="properties[additional_inventory]" value="{{ additional_inventory_time }}"><input type="hidden" name="order_date" value="{{ date }}"><input type="hidden" name="location" value="{{ location }}"><input type="hidden" name="day" value="{{ day_name }}"><button data-variant-id="{{ product.variants[0].id }}" data-product-id', $content);
            $content = str_replace('data-href="{{ product.url | within: collection }}"', 'data-href="{{ product.url | within: collection }}?location={{ location }}&no_station={{ no_station }}&additional_inventory_time={{ additional_inventory_time }}&day={{ day_name }}&uuid={{ uuid }}"', $content);
            $content = str_replace('data-href="{{ product.url }}"', 'data-href="{{ product.url }}?location={{ location }}&no_station={{ no_station }}&additional_inventory_time={{ additional_inventory_time }}&day={{ day_name }}&uuid={{ uuid }}"', $content);
