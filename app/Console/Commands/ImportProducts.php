...
class ImportProducts extends Command
{
    ...
    public function importProduct($api, $product){
        ...
        echo "Product: {$product['title']} has been imported successfully" . PHP_EOL;
    ...
    public function importProductMetafields($api, $product){
        ...
        if(count($metafields)){
            foreach ($metafields as $field) {
                $arrMetafields = Metafields::updateOrCreate(['product_id' => $product['id'], 'metafield_id' => $field['id']], [
                    'product_id' => $product['id'],
                ...
                echo "Metafield {$field['key']} for product: {$product['title']} has been imported successfully" . PHP_EOL;
            }
            echo PHP_EOL;
        }
    }
