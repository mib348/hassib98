...
/**
 * Artisan command to create an automatic yesterday item discount via Shopify GraphQL API
 *
 ...
 *
 * The discount function (yesterday-items-discount) must already be deployed to Shopify
 * and the app must have write_discounts scope enabled in shopify.app.toml
 */
class CreateYesterdayItemDiscount extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'yesterday:create-discount {functionId? : Optional Shopify Function ID (GID). If not provided, will auto-detect.}';
     ...
     * @var string
     */
    protected $description = 'Create automatic yesterday item discount (50% off) via Shopify GraphQL API';

    /**
     * Execute the console command.
     *
     * This command creates an automatic discount that:
     * - Auto-detects the yesterday items function ID or uses the provided GID
     * - Starts immediately upon creation
     * - Combines with product discounts only (not order or shipping discounts)
     * - Applies to items with yesterday_item="Y" line attribute
    ...
    public function handle()
    {
        try {
            ...
            $this->info('Creating automatic yesterday item discount via GraphQL...');
            $this->line('');
            ...
            if (!$functionId) {
                $this->info('No function ID provided. Searching for yesterday items function...');
                $functionId = $this->findYesterdayItemsFunctionId($api);

                if (!$functionId) {
                    $this->error('Could not find yesterday items function automatically.');
                    $this->line('');
                    $this->info('Please run: php artisan shopify:list-functions');
                    $this->info('Then use: php artisan yesterday:create-discount "FUNCTION_ID"');
                    return Command::FAILURE;
                }

                $this->info('✓ Found yesterday items function: ' . $functionId);
                $this->line('');
            }
            ...
            // For yesterday items 50% discount, we use PRODUCT to discount individual items
            $mutation = '
                mutation {
                    discountAutomaticAppCreate(
                        automaticAppDiscount: {
                            title: "Yesterday Items - 50% Off"
            ...
            if (isset($response['errors']) && !empty($response['errors'])) {
                ...
                Log::error('GraphQL error creating yesterday item discount: ' . $errorMessage);
                $this->error('Failed to create yesterday item discount due to GraphQL error:');
            ...
            if (isset($data['userErrors']) && !empty($data['userErrors'])) {
                ...
                Log::error('User errors creating yesterday item discount: ' . $userErrors);
                $this->error('Failed to create yesterday item discount:');
            ...
            if (isset($data['automaticAppDiscount'])) {
                ...
                Log::info('Yesterday item discount created successfully', [
                ...
                $this->info('✓ Yesterday item discount created successfully!');
                $this->line('');
                ...
                $this->info('and apply 50% discounts to items with yesterday_item="Y" line attribute.');

                return Command::SUCCESS;
            }

            // If we reach here, something unexpected happened with the response structure
            Log::error('Unexpected response creating yesterday item discount: ' . json_encode($response));
            ...
            Log::error('Exception creating yesterday item discount: ' . $th->getMessage(), [
                'exception' => $th,
                'trace' => $th->getTraceAsString(),
            ]);
            $this->error('Failed to create yesterday item discount: ' . $th->getMessage());
            $this->line('');
    ...
    protected function findYesterdayItemsFunctionId($api)
    {
        try {
            ...
            // Search for yesterday items function - look for "yesterday" in the title (case-insensitive)
            foreach ($data['nodes'] as $function) {
                $title = strtolower($function['title'] ?? '');
                if (str_contains($title, 'yesterday')) {
                    return $function['id'];
                }
            }
            ...
            Log::error('Exception finding yesterday items function: ' . $th->getMessage());
            return null;
        }
    }
}
