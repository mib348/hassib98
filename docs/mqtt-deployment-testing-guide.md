...
Shopify Order Webhook
  ...
  OrdersCreateJob
        |
        |---- new order ----------> {env}/location/{slug}/orders/new

Shopify Cancelled Webhook
        |
        v
  OrdersCancelledJob
        |
        |---- cancelled order ----> {env}/location/{slug}/orders/cancelled

Shopify Updated Webhook
        |
        v
  OrdersUpdatedJob
        |
        |---- changed order ---------> {env}/location/{slug}/orders/updated

MQTT Broker (Mosquitto)  --->  RPi Device
        ^                          |
        |                          |
  RPi connects with           RPi sends fulfillment
  ClientId = location slug         |
                                   v
...
## MQTT Client IDs and Topic Contract

Each Raspberry Pi must set its MQTT `ClientId` to the location slug it owns.
The dashboard displays this received `ClientId` as the location identity.
If a client connects without a `ClientId`, Mosquitto generates an `auto-*`
value and the dashboard marks it as autogenerated.

Examples:

| Location | MQTT ClientId | Subscribe for server events | Publish fulfillment | Publish Pi status |
|---|---|---|---|---|
| Standort 1 | `standort_1` | `{env}/location/standort_1/orders/new`, `{env}/location/standort_1/orders/cancelled`, `{env}/location/standort_1/orders/updated` | `{env}/location/standort_1/orders/fulfilled` | `{env}/location/standort_1/pi/status` |
...
`{env}` is controlled by `MQTT_TOPIC_ENV`, usually `dev` or `live`.

The Pi should publish status every 10 seconds with QoS 1 and retain=true.
Laravel subscribes to the current environment only via `{env}/location/+/pi/status`.
Node-RED listens to both `dev/location/+/pi/status` and `live/location/+/pi/status`
and marks an online device stale after 30 seconds without a new heartbeat.
...
MQTT Last Will should publish to the same topic with QoS 1 and retain=true:
...
```
mqtt:subscribe   Subscribe to MQTT topics for RPi fulfillment confirmations and Pi status heartbeats (long-running)
```
...
// Expected output: {env}/location/standort_1/orders/new

echo MqttHelper::cancelledOrderTopic("Standort 1");
// Expected output: {env}/location/standort_1/orders/cancelled

echo MqttHelper::updatedOrderTopic("Standort 1");
// Expected output: {env}/location/standort_1/orders/updated
...
2. You should see a message appear on topic `{env}/location/standort_1/orders/new`
3. Also check Laravel logs: `tail -f storage/logs/laravel.log | grep MQTT`

You should see:
```
MQTT: Published order event to topic {"topic":"{env}/location/standort_1/orders/new","event":"order.new","order_id":9999999999,...}
```

### Server -> RPi payload examples
...
```json
{
  ...
  "items": [
    {
      "product_id": 123,
      "title": "Test Sushi Box",
      "quantity": 2
    }
  ],
  "customer_name": "Test Kunde",
  "total_price": "19.90",
  "published_at": "2026-05-28T08:15:00+02:00"
}
```
...
```json
{
  ...
  "cancelled_at": "2026-05-28T08:20:00+02:00",
  "published_at": "2026-05-28T08:20:02+02:00"
}
```
...
```json
{
  ...
  "updated_at": "2026-05-28T08:25:00+02:00",
  "published_at": "2026-05-28T08:25:02+02:00"
}
```
...
Subscribed to: {env}/location/+/orders/fulfilled
Subscribed to: {env}/location/+/pi/status
```
...
docker exec mosquitto mosquitto_pub \
  -h localhost -p 1883 \
  -u mqtt_12342026 -P mqtt_12342026 \
  -t "{env}/location/standort_1/orders/fulfilled" \
  -m '{
    "order_id": 9999999999,
    "order": 9999,
...
You should see:
```
Received message on: {env}/location/standort_1/orders/fulfilled
...
**Terminal 2 — Simulate a retained Pi heartbeat message:**
...
docker exec mosquitto mosquitto_pub \
  ...
  -u mqtt_12342026 -P mqtt_12342026 \
  -q 1 -r \
  -t "{env}/location/standort_1/pi/status" \
  -m '{
    "event": "pi.status",
    "client_id": "standort_1",
    "location_slug": "standort_1",
...
Replace `{env}` with `dev` or `live`. The Laravel subscriber stores one latest
row per location in `pi_statuses`, and the Node-RED dashboard shows it in the
Pi Status table.
